...
| Página | Tamaño | Descripción |
|--------|--------|-------------|
| `LandingPage.tsx` | ~80KB | Página pública de marketing (home). Usa `LandingHeader` (header fijo unificado con mega-menú `FeaturesDropdown`, enlaces Guía/API/Login, menú hamburguesa móvil). **Footer** con enlace a "Por qué Timeboxing" (`/por-que-timeboxing`), demo, guía y API. Carousel de features con enlaces a landings comerciales. Schema JSON-LD SoftwareApplication en Helmet. Componentes: `LandingHeader.tsx`, `FeaturesDropdown.tsx`, `LandingFooter.tsx`, `CalendarPreview.tsx`. |
...
| `GuiaPage.tsx` | ~18KB | Guía de funcionalidades pública (`/guia`, `/guia/:section`). Usa `LandingHeader`. Contiene 6 componentes de contenido reutilizables (FeatureCard, ExampleBox, TipBox, WarningBox, StepList, InfoGrid), navegación prev/next entre secciones y contenido detallado para las 9 secciones del producto. |
...
| `PresentationPage.tsx` | ~18KB | Presentación interna tipo PowerPoint **oculta** en `/pitch` (noindex, no enlazada). 15 slides fullscreen con CSS mockups interactivos: Portada, Contexto, Problema, Coste, Before/After, Solución (3 pilares), 6 feature slides con mockups (Planificador+Asignación, Dashboard, Equipos, Reportes, Deadlines+Forecast, Integraciones), Seguridad/Arquitectura, ROI y CTA. Mockups en `PresentationMockups.tsx`. Navegación ← → teclado, clic lateral, swipe táctil y dots. |
