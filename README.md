...
| **`stripe-webhook`** | `supabase/functions/stripe-webhook/index.ts` | Webhook de Stripe: actualiza `agencies` (plan_id, subscription_status, trial_ends_at) según eventos de suscripción. Requiere `STRIPE_WEBHOOK_SECRET`. Opcional: `STRIPE_PRICE_ID_PRO` / `STRIPE_PRICE_ID_BUSINESS` (mismos IDs que en Vite) para inferir Pro/Business por precio. |
