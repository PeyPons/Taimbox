/** Builder PPC enriquecido (paridad con src/utils/adsPpcAlertBuild.ts). */
...
/** Alias usado por plantillas de email. */
export type AdsPpcAlert = AdsPpcAlertDetail;
...
function buildPlatformAlerts(
  ...
  const clientSettings: Record<string, { budget: number; group_name: string; is_hidden: boolean }> = {};
  ...
  for (const row of rows) {
    const rowDate = typeof row.date === "string"
  ...
  for (const [key, value] of stats) {
    ...
    alerts.push({
      ...
      accountRefs: value.realIdsNames.length ? value.realIdsNames : [{ id: key, name: value.name }],
      status,
      monthKey,
...
export function computeAdsPpcAlerts(
  settings: AdsClientSettingRow[],
  googleRows: AdsCampaignRow[],
  metaRows: AdsCampaignRow[],
  platforms: Array<"google" | "meta">,
  now: Date = new Date(),
): AdsPpcAlertDetail[] {
  ...
  return buildAdsPpcAlerts({
    settings,
    google: useGoogle ? { campaigns: googleRows, accounts: [], rules: [] } : undefined,
    meta: useMeta ? { campaigns: metaRows, accounts: [], rules: [] } : undefined,
    platforms,
    now,
  });
}
