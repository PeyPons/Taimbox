...
export function isAllocationInEffectiveMonthForNotifications(weekStartDate: string, viewMonth: Date): boolean {
  try {
...
export function getEffectiveCompletedHoursForNotifications(
  alloc: AllocationRow,
...
export function analyzeProjectMonthForNotifications(
  project: ProjectRow,
  ...
  const monthTasks = allocations.filter(
    (a) =>
      a.project_id === project.id && isAllocationInEffectiveMonthForNotifications(a.week_start_date, viewMonth),
  );
  ...
  const hoursComputed = completedTasks.reduce(
    (sum, t) => sum + getEffectiveCompletedHoursForNotifications(t, hoursPreference),
    0,
  );
