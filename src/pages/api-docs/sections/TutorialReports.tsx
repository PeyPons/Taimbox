...
export function TutorialReports() {
  ...
  const steps = i18nAsArray<TutorialStepJson>(t('tutorials.reports.steps', { returnObjects: true }));

  return (
    <section>
      <SectionHeading id="tutorial-reports" icon={BarChart3} className="mb-2">
        {t('tutorials.reports.title')}
      </SectionHeading>
      <p className="text-indigo-100/85 mb-6">
        {t('tutorials.reports.subtitle')}
      </p>
      ...
      <div className="mt-6 p-4 rounded-lg bg-emerald-500/10 border border-emerald-500/20">
        <p className="text-sm text-emerald-100/90">
          <strong className="text-emerald-300">{t('tutorials.reports.nextLabel')}</strong>{' '}
          {t('tutorials.reports.nextText')}{' '}
          <button
            type="button"
          ...
          >
            {t('tutorials.reports.nextLink')}
          </button>
          {t('tutorials.reports.nextSuffix')}
        </p>
      </div>
    </section>
  );
