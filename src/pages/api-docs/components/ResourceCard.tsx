        ...
        <CardContent className="p-5 space-y-6">
          <div>
            <h5 className="text-white font-semibold text-sm mb-3 flex items-center gap-2">
              <ChevronRight className="h-3.5 w-3.5 text-indigo-400" /> {t('resourceCard.columns')}
            </h5>
            <ParamTable columns={table.columns} />
          </div>

          <div>
            <h5 className="text-white font-semibold text-sm mb-3">{t('resourceCard.responseExamples')}</h5>
            <ResponseExample
          ...
          <div>
            <div className="flex items-center justify-between gap-2 mb-2">
              <h5 className="text-white font-semibold text-sm">{t('resourceCard.usageExamples')}</h5>
            ...
            >
              <div className={exampleLang === 'curl' ? 'min-w-0' : ''}>
                <div className="text-white/70 text-xs font-medium mb-1.5 flex items-center gap-2">
                  <MethodBadge method="GET" /> <span>{t('resourceCard.fetch')}</span>
                </div>
              ...
              <div className={exampleLang === 'curl' ? 'min-w-0' : ''}>
                <div className="text-white/70 text-xs font-medium mb-1.5 flex items-center gap-2">
                  <MethodBadge method="POST" /> <span>{t('resourceCard.create')}</span>
                </div>
                {exampleLang === 'typescript' ? (
