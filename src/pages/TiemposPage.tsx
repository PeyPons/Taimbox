...
export default function TiemposPage() {
  ...
  const handleStopOwn = async () => {
    ...
    toast({ title: t('times.toast.stoppedTitle'), description: t('times.toast.stoppedDescription') });
  };
  ...
  if (!isTimeTrackerEnabled) {
    return (
      <div className="flex flex-col h-full space-y-6 p-6 md:p-8 max-w-4xl mx-auto w-full">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-slate-900">{t('times.title')}</h1>
          <p className="text-muted-foreground mt-1">{t('times.subtitleDisabled')}</p>
        </div>
        <Card>
          <CardContent className="pt-6">
            <p className="text-slate-600">{t('times.disabledMessage')}</p>
          </CardContent>
        </Card>
      </div>
    );
  ...
  return (
    <div className="flex flex-col h-full space-y-6 p-6 md:p-8 max-w-4xl mx-auto w-full">
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-slate-900">{t('times.title')}</h1>
        <p className="text-muted-foreground mt-1">{t('times.subtitleActive')}</p>
      </div>
      ...
      {isLoading ? (
        ...
        <Card>
          <CardHeader>
            ...
            <CardTitle className="text-center">{t('times.emptyState.title')}</CardTitle>
            <CardDescription className="text-center">
              {t('times.emptyState.description')}
            </CardDescription>
          </CardHeader>
          {currentUser && (
            <CardContent className="pt-0 border-t">
              <div className="flex items-center justify-between pt-4">
                <span className="text-sm text-slate-600">{t('times.emptyState.yourTotalToday')}</span>
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 text-slate-900 font-medium">
                        ...
                        <span>{row.employee_name ?? t('times.cards.defaultEmployee')}</span>
                        {isOwn && (
                          <span className="text-xs bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded">{t('times.cards.you')}</span>
                        )}
                      </div>
                      <div className="mt-1 text-sm text-slate-600 truncate">
                        {row.task_name ?? t('times.cards.defaultTask')}
                        {row.client_name && (
                      ...
                      {isOwn && activeTimer.formattedTimeLabel !== '0 min' && (
                        <div className="mt-1 text-xs text-slate-500">
                          {t('times.cards.todayTotal')}: {activeTimer.formattedTimeLabel}
                        </div>
                      )}
                    </div>
            ...
            <Card className="border-dashed">
              <CardContent className="py-4">
                <div className="flex items-center justify-between">
                  <span className="text-sm text-slate-600">{t('times.cards.yourTotalWithoutTimer')}</span>
                  <span className="text-sm font-medium text-slate-900">{activeTimer.formattedTimeLabel}</span>
                </div>
              </CardContent>
            </Card>
