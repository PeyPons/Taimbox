...
export const LandingBelowSection07: FC = () => {
  ...
  const { t: tPricing } = useTranslation("landing");

  const plansById = Object.fromEntries(

    i18nAsArray<Plan>(t("s07.plans", { returnObjects: true })).map((p) => [p.id, p]),

  ) as Record<PublicPlanId, Plan | undefined>;
    ...
    >
      ...
      <div className="relative max-w-7xl mx-auto px-6 lg:px-10">
        ...
        >
          ...
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-amber-400/15 border border-amber-500/35 text-amber-900 text-xs font-semibold uppercase tracking-wider">

            <Rocket className="h-3.5 w-3.5 text-amber-600 shrink-0" aria-hidden />
        ...
        <p className="text-center text-[11px] text-slate-500 max-w-xl mx-auto -mt-4 mb-10">{billingNote}</p>



        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-5">

          {PUBLIC_PLAN_PRICING.map((pricing) => {

            const plan = plansById[pricing.id];
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center justify-between gap-3 mb-4">
                  ...
                  {plan.discountBadge && hasEarlyDiscount ? (

                    <span

                      className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[9.5px] font-bold tracking-[0.12em] uppercase shrink-0 border border-amber-400/40 text-amber-100 bg-amber-500/25"
                  ...
                  {hasEarlyDiscount ? (
                    ...
                    <p
                      ...
                      className={`mt-1.5 text-[10.5px] font-semibold uppercase tracking-wide ${
                        ...
                        featured ? "text-amber-300/95" : "text-amber-700"
