...
import { ChevronLeft, ChevronRight, CalendarDays, AlertCircle, CheckCircle2, ListPlus } from 'lucide-react';
...
export function DemoEmployeeDashboard() {
  ...
  return (
    <div className="max-w-7xl mx-auto p-3 sm:p-4 md:p-6 space-y-4 sm:space-y-6 pb-20">
      {/* Banner demo: una línea discreta */}
      <p className="text-xs text-slate-500 text-center sm:text-left">Demo con datos de ejemplo. Solo lectura.</p>

      {/* CONTROL MES - mismo estilo que el dashboard real */}
      ...
      {/* CALENDARIO: móvil = tarjetas (MobilePlannerView); desktop = tabla */}
      {isMobile ? (
      ...
      {/* PESTAÑAS - mismo orden y estilo que el dashboard real */}
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="w-full justify-start h-auto p-1 bg-white border border-slate-200 flex-nowrap overflow-x-auto custom-scrollbar gap-2 min-w-0">
          ...
          <TabsTrigger value="teammates" className="px-4 py-2 min-h-[44px] shrink-0">Compañeros</TabsTrigger>
          <TabsTrigger value="metrics" className="px-4 py-2 min-h-[44px] shrink-0">Mis métricas</TabsTrigger>
        </TabsList>
