...
const OPTIONS: {
...
}[] = [
  {
    mode: 'give',
    preset: 'prudent',
    title: 'Dar horas a alguien',
    description: 'Elige quién necesita carga. Por defecto: solo proyectos en común y quienes comparten proyecto.',
    icon: UserPlus,
  },
  {
    mode: 'take',
    preset: 'prudent',
    title: 'Quitar carga a alguien',
    description: 'Elige quién va sobrecargado y a quién puede pasarle horas en proyectos compartidos.',
    icon: UserMinus,
  },
  {
    mode: 'team',
    preset: 'explore',
    title: 'Equilibrar todo el equipo',
    description: 'Vista global: varias personas, límites de carga y mínimo por movimiento.',
    icon: LayoutGrid,
  },
];
...
const QUICK_PRESETS: {
  ...
  { preset: 'prudent', mode: 'give', title: 'Reparto prudente (dar)', icon: Shield },
  { preset: 'heavy', mode: 'take', title: 'Solo muy cargados ceden', icon: Gauge },
];
  ...
  return (
    <div className="space-y-4 py-1">
      <div className="flex items-start gap-3 rounded-xl border border-primary/20 bg-primary/5 p-3">
        ...
        <div>
          <p className="text-sm font-semibold text-slate-800">¿Qué quieres hacer?</p>
          <p className="text-xs text-slate-600 mt-0.5">
            Te guiamos paso a paso: persona → reglas (quién cede / proyectos) → detalle. Los cambios los aplicas en
            cada proyecto.
          </p>
        </div>
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                {highlighted && (
                  <p className="text-[10px] text-primary font-medium mt-1">Usado la última vez</p>
                )}
              </div>
            </button>
          );
