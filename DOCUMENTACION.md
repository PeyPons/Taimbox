...
- **Librería**: `i18next` + `react-i18next`; configuración en `src/i18n/config.ts` (recursos `landing`, `blog`, `app`; español por defecto).
...
- **Etiquetas SEO**: Preferir `src/seo/SeoTags.tsx` (título, descripción, `canonical` absoluto vía `getPublicSiteUrl` / `VITE_PUBLIC_SITE_URL`, `hreflang` `es` / `en` / `x-default`, OG y JSON-LD opcional). Helpers: `src/seo/CommercialSeoTags.tsx` (landings de producto), `src/seo/BlogArticleSeo.tsx` (posts del blog con JSON-LD existente).
- **Traducciones UI**: JSON en `src/locales/es|en/`; header/footer/cookies/precios y páginas estáticas usan claves bajo `landing` (p. ej. `static.*`, `commercial.*`, `guide.*`). La home (`LandingPage.tsx`) usa `home.*`: carrusel de funcionalidades (`home.featuresCarousel`, iconos y colores fijos en código), bloque integraciones (`home.integrationsCards`), FAQ (`home.faqItems`; la entrada con enlace a API usa `answerBefore` / `answerLinkText` / `answerAfter` / `answerLinkPath`). Las landings de producto con artículo largo usan `commercial.<producto>.page` (p. ej. `employeeDashboard`, `planner`, `team`, `reports`, `projects`, `integrations` en los `*Article.tsx` correspondientes). Las landings monolíticas **Seguridad** (`SecurityLandingPage.tsx`) y **Monitor PPC** (`PpcMonitorLandingPage.tsx` + `PpcMonitorPacingChart.tsx`) usan `commercial.security.page` y `commercial.ppc.page` (incl. `commercial.ppc.page.chart` para el gráfico de pacing). El panel usa namespace `app` y `src/hooks/useAppTranslation.ts` (aún por ampliar en pantallas internas). **Lista de subtareas pendientes** (landings monolíticas, blog, guía, panel, API docs): `docs/i18n-tareas-restantes.md`.
- **`returnObjects` y arrays**: Si `t(key, { returnObjects: true })` se usa con `.map()` (o se asume array), envolver el resultado con `i18nAsArray` de `src/lib/i18nReturnObjects.ts`: cuando falta la clave o i18next devuelve el path como string, sin eso aparece el error en runtime `… is not a function` (p. ej. `map`). Aplica a landings comerciales, precios, home, guía (`GuiaPage` → p. ej. `weeklyForecast.howSteps`) y cualquier lista traducida.
...
| WeeklyForecastPage | `src/pages/WeeklyForecastPage.tsx` | ~1250. Capa de orquestación UI sobre hooks de dominio: `useWeeklyForecastMonthData`, `useWeeklyForecastFilters`, `useWeeklyForecastProjectForecast`, `useWeeklyForecastTransfers`, `useWeeklyForecastRedistribution`. |
