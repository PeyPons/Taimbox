...
| WeeklyReportDialog | `src/components/employee/WeeklyReportDialog.tsx` | Modal Weekly (UI corporativa: cabecera, buscador, tarjetas por tarea, **cuatro** acciones con copy formal, paneles de detalle unificados, pie “Cancelar” / **Confirmar cierre**). **Prop opcional `focusAllocationId`:** desde el planificador (`AllocationSheet` + menú ⋯) abre el mismo modal centrado en una allocation; la tarea se **inyecta** en la lista si aún no entra en el filtro de semanas cerradas / mes. Con una sola tarea en ese contexto: sin sidebar (desktop) ni selector móvil, título *Opciones Weekly* (vista compacta). **Posponer:** **Posponer lo pendiente** (saldo automático → `applyRollover`). **Mutaciones:** `applyMoveToEmployee`, `applyJustify`, `applyKeep`, `applyRollover`, `applyDistribute` vía `useWeeklyCloseMutations`. **Semanas destino:** mes de `viewDate` + mes siguiente. Al abrir: `ensureMonthLoaded` + `loadDataForMonth(mes siguiente)`. **Layout master-detail** cuando hay varias tareas. Validación: `canSubmit`, `validationErrors`, `capacityWarnings`. |
...
| `useWeeklyCloseMutations.ts` | `WeeklyReportDialog.tsx`, `TaskPartialCloseDialog.tsx` (mutaciones compartidas de cierre Weekly / parcial; exporta `WEEKLY_SLOT_EXTRA_MONTHS`, `parseWeeklyCloseHours`, `normalizeWeeklyHourInput`) |
...
- **Toasts (Sonner)**: `src/lib/notify.ts` es la fachada; el `<Toaster />` está en `src/components/ui/sonner.tsx` (posición `bottom-right`, `z-index` alto para verse sobre `Dialog`). Import `toast` desde `@/lib/notify` o `useToast` / `toast` legacy en `@/hooks/use-toast`.
