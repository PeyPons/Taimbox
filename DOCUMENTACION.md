...
| [docs/15-cms-blog.md](docs/15-cms-blog.md) | CMS de blog en `/admin/blog` (tabla `blog_posts`, RLS `platform_admins`, bloques JSONB tipados con Zod, `visualRegistry` para reusar los `*Article.tsx` actuales como `fullPage`, sanitizado DOMPurify, rutas dinámicas `/blog/:slug` y `/en/blog/:slug`, `BlogPathSync` y sitemap auto). |
