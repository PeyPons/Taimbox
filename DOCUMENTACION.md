...
| [docs/07-mantenimiento-extension.md](docs/07-mantenimiento-extension.md) | Cómo extender tablas, cronómetro (drift, eventos, **Real vs `time_entries`**, página Tiempos y `ModuleGuard`), RLS y API tokens, admin de plataforma (`admin_delete_agency`, borrado irreversible), operaciones, **rentabilidad (gastos comunes recurrentes y puntuales, coste cargado, EHR en pantalla Rentabilidad)** y permisos; cierre parcial / rollover (incl. migración `20260413120000` para posponer con 0h reales). |
| [docs/08-mapa-dependencias.md](docs/08-mapa-dependencias.md) | Mapa de impacto por types, contextos, hooks, utilidades (**`commonExpensesAllocation`**), componentes y flujo de carga de mes. |
